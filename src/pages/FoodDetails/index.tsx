...
const FoodDetails: React.FC = () => {
  ...
  useEffect(() => {
    async function loadFood(): Promise<void> {
      ...
      setExtras(
        foodResponse.extras.map((extra: Extra) => ({
          ...extra,
          quantity: 0,
        })),
      );
  ...
  useEffect(() => {
    async function loadFavorite(): Promise<void> {
      ...
      const favorite: Favorite = response.data;
      if (favorite) {
        setIsFavorite(true);
      }
    }
  ...
  const cartTotal = useMemo(() => {
    ...
    const totalExtras = extras.reduce((accumulative, extra) => {
      return accumulative + extra.quantity * extra.value;
    }, 0);
